...
const fs = require("fs");
const marnel = require("./ayarlar/marnel.json");
...
let ranks = ["normal", "altin", "elmas", "hazir", "html", "ekbilgi", "topluluk", "api"];
for (let rank in ranks) {
  db[ranks[rank]] = new bookman(ranks[rank]);
}

const IDler = {
  botID: "809862096211673088",
  botSecret: "2VR_5WWRo2FQtLc49tuZDZDCoZ08Ov-U",
  botCallbackURL: "https://sharp-translucent-wealth.glitch.me/callback",
  sunucuID: "806917768355381318",
  sunucuDavet: "https://discord.gg/RqRAJPbQ6p",
  kodLogKanalı: "813759035202994186",
  sahipRolü: "806928215339565077",
  adminRolü: "809863448866324490",
  kodPaylaşımcıRolü: "813806813400793108",
  boosterRolü: "808043127536877568",
  kodPaylaşamayacakRoller: ["809773113797640202", "809773113797640202"],
  hazırAltyapılarRolü: "813726046447730698",
  hazırSistemlerRolü: ["809415430279004170","810211813932793886"],
  htmlKodlarRolü: "810211813932793886",
  ekbilgirolü: "809863448866324490",
  elmasKodlarRolü: "809415430279004170",
  altınKodlarRolü: "813725199459287090",
  normalKodlarRolü: "809415430279004170"
};
...
app.get("/ekbilgi", (req, res) => {
  var data = db.ekbilgi.get("kodlar");
  data = sortData(data);
  res.render("ekbilgi", {
    user: req.user,
    kodlar: data
...
app.get("/ekbilgi/:id", (req, res) => {
  ...
  var id = req.params.id;
  if (!id) req.redirect("/");
  let data = db.ekbilgi.get("kodlar");
  var code = findCodeToId(data, id);
  if (code) {
    let guild = client.guilds.cache.get(IDler.sunucuID);
    let member = req.user ? guild.members.cache.get(req.user.id) : null;
    if (
      member &&
      (member.roles.cache.has(IDler.ekbilgirolü) ||
        member.roles.cache.has(IDler.boosterRolü) ||
        member.roles.cache.has(IDler.sahipRolü) ||
        member.roles.cache.has(IDler.kodPaylaşımcıRolü) ||
        member.roles.cache.has(IDler.adminRolü))
...
app.post("/paylasim", (req, res) => {
  ...
  client.channels.cache.get(IDler.kodLogKanalı).send(
    new Discord.MessageEmbed()
      ...
      .addField(
        "Kod Sayfası",
        `[Tıkla!](https://cobalt-past-cucumber.glitch.me/${obj.kod_rank}/${obj.id})`
      )
  );
...
client.on("ready", () => {
  const listener = app.listen(process.env.PORT, function() {
    client.user.setActivity(`Unity Code`, `https://cobalt-past-cucumber.glitch.me/`, {
      type: "WATCHING"
    });
